	...
	<div class="main-container">
		...
		<div class="main-content">
			...
			<main class="content">
				...
				<div class="filtros">
					<button class="filtro-btn ativo" onclick="filtrar('todos',this)">🍽️
						Todos</button>
					<c:forEach var="cat" items="${categorias}">
						<button class="filtro-btn"
							onclick="filtrar('cat-${cat.idCategoria}',this)">${cat.nome}</button>
					</c:forEach>
				</div>
				...
				<c:forEach var="cat" items="${categorias}">
					<div class="secao-categoria" id="cat-${cat.idCategoria}">
						<div class="secao-titulo">
							<span>${cat.nome}</span> <span class="setor-badge">${cat.setor}</span>
						</div>
							...
							<c:forEach var="item" items="${itens}">
								<c:if test="${item.categoriaId == cat.idCategoria}">
									<div class="item-card ${item.disponivel ? '' : 'indisponivel'}">
										...
										<span
											class="item-status ${item.disponivel ? 'status-disp' : 'status-indisp'}">
											${item.disponivel ? '✓ Disponível' : '✕ Indisponível'} </span>
										...
										<div class="item-rodape">
											<span class="item-preco"> R$ <fmt:formatNumber
													...
													maxFractionDigits="2" />
											</span> <span class="item-tempo">⏱ ${item.tempoPreparoMin}
												min</span>
										</div>
												...
												<c:choose>
													...
													<c:otherwise>
														...
														<button type="submit" class="btn btn-success btn-sm"
															style="width: 100%">✅ Liberar</button>
													</c:otherwise>
												</c:choose>
											</form>
