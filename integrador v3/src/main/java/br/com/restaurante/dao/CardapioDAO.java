...
public class CardapioDAO {
    ...
    */
    public void inserir(Cardapio i) throws SQLException {
        try(PreparedStatement s=conexao.prepareStatement(
            "INSERT INTO cardapio(categoria_id,nome,descricao,preco,tempo_preparo_min,disponivel,ativo) VALUES(?,?,?,?,?,1,1)",
            Statement.RETURN_GENERATED_KEYS)){
            s.setInt(1,i.getCategoriaId());
            s.setString(2,i.getNome());
            s.setString(3,i.getDescricao());
            s.setBigDecimal(4,i.getPreco());
            s.setInt(5,i.getTempoPreparoMin());
            s.executeUpdate();
    ...
    */
    public void editar(Cardapio i) throws SQLException {
        try(PreparedStatement s=conexao.prepareStatement(
            "UPDATE cardapio SET categoria_id=?,nome=?,descricao=?,preco=?,tempo_preparo_min=? WHERE id_cardapio=?")){
            s.setInt(1,i.getCategoriaId());
            s.setString(2,i.getNome());
            s.setString(3,i.getDescricao());
            s.setBigDecimal(4,i.getPreco());
            s.setInt(5,i.getTempoPreparoMin());
            s.setInt(6,i.getIdItem());
            s.executeUpdate();
        }
    }
