...
public class PedidoController extends HttpServlet {
    ...
    */
    private void avancarStatus(HttpServletRequest request, HttpServletResponse response)
        ...
        // ---- Pagamento(s) — agora aceita MAIS DE UMA forma por pedido
        // (ex: parte no cartão, parte em dinheiro) — ver explicação
        // completa dentro do bloco "entregue" mais abaixo.
        String[] formasParam  = request.getParameterValues("formaPagamento");
        String[] valoresParam = request.getParameterValues("valorPagamento");
        ...
        try (Connection conn = Conexao.getConnection()) {
            ...
            try {
                ...
                if ("entregue".equals(proximo)) {
                    ...
                    // ========================================================
                    // REGISTRAR PAGAMENTO(S) — agora aceita MAIS DE UM POR PEDIDO
                    // ========================================================
                    // form_pedido/detalhe_pedido.jsp enviam formaPagamento[] e
                    // valorPagamento[] (arrays paralelos, mesma posição = mesma
                    // linha da UI) — o cliente/gerente pode adicionar quantas
                    // linhas quiser na hora de fechar a conta (ex: metade no
                    // cartão, metade em dinheiro).
                    //
                    // REGRAS DE PREENCHIMENTO AUTOMÁTICO DE VALOR (mesma ideia
                    // de antes, agora generalizada para várias linhas):
                    //   - Uma linha com forma escolhida MAS valor vazio/inválido
                    //     é marcada como "automática" — no máximo UMA linha pode
                    //     ficar assim; ela recebe o que SOBROU do total do
                    //     pedido depois de somar as linhas com valor explícito.
                    //   - Se mais de uma linha ficar sem valor ao mesmo tempo,
                    //     não tem como adivinhar como dividir o restante entre
                    //     elas — essas linhas extras são IGNORADAS (log de aviso),
                    //     só a primeira "automática" é preenchida.
                    //   - Linhas sem forma selecionada (branco) são ignoradas
                    //     silenciosamente — é o comportamento normal de linhas
                    //     extras não usadas na UI.
                    if (formasParam != null && formasParam.length > 0) {

                        BigDecimal totalPedido = calcularTotalPedido(conn, id);
                        BigDecimal somaExplicita = BigDecimal.ZERO;
                        int idxAutomatica = -1; // posição da 1ª linha sem valor explícito, se houver
                        List<BigDecimal> valoresResolvidos = new ArrayList<>();

                        // ---- Primeira passada: parse dos valores explícitos ----
                        for (int i = 0; i < formasParam.length; i++) {
                            String forma = formasParam[i];
                            if (forma == null || forma.isBlank()) {
                                valoresResolvidos.add(null); // linha não usada
                                continue;
                            }
                            String valorStr = (valoresParam != null && i < valoresParam.length) ? valoresParam[i] : null;
                            BigDecimal valor = null;
                            if (valorStr != null && !valorStr.isBlank()) {
                                try { valor = new BigDecimal(valorStr.replace(",", ".")); } catch (Exception ignored) {}
                            }
                            if (valor != null && valor.compareTo(BigDecimal.ZERO) > 0) {
                                somaExplicita = somaExplicita.add(valor);
                                valoresResolvidos.add(valor);
                            } else if (idxAutomatica == -1) {
                                idxAutomatica = i; // primeira linha "em aberto" — preenche depois
                                valoresResolvidos.add(null);
                            } else {
                                System.out.println("⚠️ Linha de pagamento #" + i + " (" + forma
                                        + ") sem valor e já existe outra automática — ignorada");
                                valoresResolvidos.add(BigDecimal.ZERO); // marca para pular no insert
                            }
                        }

                        // ---- Preenche a única linha automática com o restante ----
                        if (idxAutomatica != -1) {
                            BigDecimal restante = totalPedido.subtract(somaExplicita);
                            if (restante.compareTo(BigDecimal.ZERO) <= 0) restante = BigDecimal.ZERO;
                            valoresResolvidos.set(idxAutomatica, restante);
                        }

                        // ---- Insere uma linha de pagamento por forma válida ----
                        int qtdRegistrados = 0;
                        BigDecimal totalRegistrado = BigDecimal.ZERO;
                        for (int i = 0; i < formasParam.length; i++) {
                            String forma = formasParam[i];
                            BigDecimal valor = valoresResolvidos.get(i);
                            if (forma == null || forma.isBlank() || valor == null
                                    || valor.compareTo(BigDecimal.ZERO) <= 0) {
                                continue; // linha vazia/ignorada/zerada
                            }
                            ...
                            qtdRegistrados++;
                            totalRegistrado = totalRegistrado.add(valor);
                            System.out.println("💰 Pagamento " + qtdRegistrados + " registrado: " + forma + " — R$ " + valor);
                        }

                        if (qtdRegistrados == 0) {
                            System.out.println("ℹ️ Nenhuma forma de pagamento válida informada — entrega registrada sem pagamento");
                        } else if (totalRegistrado.compareTo(totalPedido) != 0) {
                            // Não bloqueia — só avisa no log. Pagamento parcial
                            // pode ser intencional (conta fiado, acerto depois
                            // etc.); quem decide se isso é aceitável é o
                            // operador, não o sistema.
                            System.out.println("⚠️ Total pago (R$ " + totalRegistrado
                                    + ") difere do total do pedido (R$ " + totalPedido + ")");
                        }
                    } else {
