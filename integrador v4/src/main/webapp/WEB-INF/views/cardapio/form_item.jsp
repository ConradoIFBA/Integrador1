...
.upload-imagem{
  display:flex;align-items:center;gap:16px;padding:14px;border:1.5px dashed var(--border-subtle-2);
  border-radius:var(--radius-sm);background:var(--bg-input)
}
.upload-preview{
  width:88px;height:88px;border-radius:var(--radius-sm);object-fit:cover;flex-shrink:0;
  background:var(--bg-card-hover);display:flex;align-items:center;justify-content:center;
  font-size:28px;color:var(--text-muted);border:1px solid var(--border-subtle)
}
.upload-preview img{width:100%;height:100%;object-fit:cover;border-radius:var(--radius-sm)}
.upload-info{flex:1}
.upload-info .dica{font-size:12px;color:var(--text-secondary);margin-top:4px}
			...
			<main class="content">
				<div class="card form-card">
					<form method="POST"
						...
						<div class="form-group">
							...
							<div class="upload-imagem">
								<div class="upload-preview" id="previewBox">
									<c:choose>
										<c:when test="${not empty item && item.temImagem}">
											<img id="previewImg" src="${pageContext.request.contextPath}/imagens/cardapio/${item.imagem}" alt="">
										</c:when>
										<c:otherwise>
								...
								<div class="upload-info">
									<input type="file" name="imagem" accept="image/jpeg,image/png,image/webp" onchange="previewImagem(this)">
									<div class="dica">JPG, PNG ou WEBP — até 3 MB.
						...
						<div class="form-group">
							<label for="categoriaId">Categoria <span
								style="color: var(--error-color)">*</span></label> <select id="categoriaId"
						...
						<div class="form-group">
							<label for="nome">Nome do item <span
								style="color: var(--error-color)">*</span></label> <input type="text" id="nome"
								name="nome" placeholder="Ex: Frango grelhado"
						...
						<div class="form-row">
							<div class="form-group">
								<label for="preco">Preço (R$) <span
									style="color: var(--error-color)">*</span></label> <input type="number" id="preco"
									name="preco" placeholder="0.00" step="0.01" min="0.01"
									value="${not empty item ? item.preco : ''}" required>
							</div>
							<div class="form-group">
