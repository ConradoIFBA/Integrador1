...
<%--
    ================================================================
    DELIVERY.JSP — MONTAR PEDIDO DE DELIVERY (v4 — tema escuro)
    ================================================================
    Restilizado para o tema escuro do style.css v4 (reaproveitando
    .pill-filtro, .card, .badge etc.), mas MANTIVE o layout em LISTA
    (item + botão −/qtd/+ ao lado) em vez de virar um grid de cards
    com imagem como o cardapio.jsp — de propósito: aqui o usuário
    está montando um carrinho e comparando preço/quantidade de
    vários itens ao mesmo tempo, então uma lista densa e escaneável
    funciona melhor que cards grandes (é o mesmo motivo pelo qual
    apps de delivery reais — iFood, Rappi — usam lista, não grid,
    na tela de "adicionar ao carrinho", mesmo tendo fotos).

    TODA A LÓGICA JS (sacola em memória, cálculo de total, geração
    dinâmica dos filtros por categoria, envio via inputs ocultos no
    submit) continua EXATAMENTE IGUAL — só as cores/classes CSS
    foram trocadas para o tema escuro.
    ================================================================
--%>
<%
...
.item-linha{
  ...
  border:1px solid var(--border-subtle);border-radius:var(--radius-sm);margin-bottom:8px;
  transition:var(--transition);background:var(--bg-card)
}
.item-linha:hover{border-color:var(--border-subtle-2)}
.item-linha.selecionado{border-color:var(--primary);background:var(--primary-light)}
.item-nome{font-size:14px;font-weight:600}
.item-preco{font-size:13px;color:var(--success-color);font-weight:700}
.item-cat{font-size:12px;color:var(--text-muted)}

.qtd-controle{display:flex;align-items:center;gap:10px}
.qtd-btn{
  width:28px;height:28px;border-radius:50%;border:1.5px solid var(--border-subtle-2);
  background:var(--bg-card-hover);color:var(--text-primary);font-size:16px;font-weight:700;
  cursor:pointer;display:flex;align-items:center;justify-content:center;transition:var(--transition)
}
.qtd-btn:hover{border-color:var(--primary);color:var(--primary)}
.qtd-val{font-size:15px;font-weight:700;min-width:20px;text-align:center}

.sacola{position:sticky;top:20px}
.sacola-item{display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border-subtle);font-size:13px}
.sacola-item:last-child{border-bottom:none}
.sacola-total{display:flex;justify-content:space-between;padding-top:12px;font-size:16px;font-weight:800;border-top:1px solid var(--border-subtle-2);margin-top:6px}
.vazia{text-align:center;padding:24px;color:var(--text-muted);font-size:13px}
.secao-titulo{font-size:15px;font-weight:700;margin-bottom:14px;padding-bottom:10px;border-bottom:1px solid var(--border-subtle)}
        ...
        <div>
          <div class="card">
            ...
            <div class="filtros" id="filtrosCat">
              <button type="button" class="pill-filtro ativo" onclick="filtrar('todos',this)">Todos</button>
            </div>
            ...
            <c:choose>
              ...
              <c:otherwise>
                <c:forEach var="item" items="${itens}">
                  <div class="item-linha" id="card-${item.idItem}"
        ...
        <div class="sacola">
          <div class="card">
            ...
            <div class="sacola-total" id="sacolaTotal" style="display:none">
              <span>Total</span>
              <span id="valorTotal" style="color:var(--success-color)">R$ 0,00</span>
            </div>
...
function renderSacola() {
  ...
  for (var id in sacola) {
    ...
    html += '<div class="sacola-item"><span>' + it.qtd + 'x ' + it.nome + '</span>'
          + '<span style="font-weight:700;color:var(--success-color)">R$ '
...
function filtrar(cat, btn) {
  ...
  document.querySelectorAll('.item-linha').forEach(el => {
    el.style.display = (cat === 'todos' || el.dataset.cat == cat) ? '' : 'none';
  });
}

// Gera um botão de filtro por categoria dinamicamente, uma vez por
// categoria distinta encontrada entre os itens renderizados —
// inalterado em relação à versão anterior.
window.addEventListener('DOMContentLoaded', function() {
  var cats = {}, cont = document.getElementById('filtrosCat');
  document.querySelectorAll('.item-linha').forEach(function(el) {
