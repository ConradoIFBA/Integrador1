...
<%--
    ...
    ================================================================
    A versão anterior já tinha uma "barra de progresso", mas era só
    texto em linha (Recebido › Preparando › Pronto › Entregue) sem
    nenhum elemento visual de círculo/linha conectora. O Figma
    mostra explicitamente uma linha do tempo HORIZONTAL de verdade:
    círculos numerados ligados por uma linha, com ✓ nos passos já
    concluídos e destaque (verde, levemente pulsante) no passo
    atual — foi isso que implementei abaixo em .timeline-h.

    Cada pedido continua sendo UM CARD (esta página lista TODOS os
    pedidos do cliente, não só um) — o Figma mostra uma tela de
    "Acompanhar" dedicada a UM pedido por vez, mas nosso Controller
    (ClienteController → meusPedidos()) já retorna a LISTA completa
    de pedidos do cliente de uma vez, então mantive "um card por
    pedido, cada um com sua própria timeline" — dá o mesmo
    resultado visual, só que permite ver vários pedidos na mesma
    tela sem precisar navegar entre eles.

    TODA A LÓGICA (auto-refresh 30s, quais status pulam a timeline —
    cancelado — e o cálculo de qual passo é "feito"/"ativo"/"futuro")
    continua a mesma; só reescrevi o HTML/CSS da timeline em si.
    ================================================================
--%>
<%
...
.refresh-bar a{color:var(--primary);text-decoration:none}

.pedido-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:8px}
.pedido-id{font-size:16px;font-weight:800}
.pedido-itens{font-size:13px;color:var(--text-secondary);background:var(--bg-card-hover);padding:10px 14px;border-radius:var(--radius-sm);margin-bottom:14px}
.pedido-footer{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px}
.pedido-total{font-size:15px;font-weight:700;color:var(--success-color)}
...
.timeline-h{display:flex;margin-bottom:16px}
.th-step{flex:1;text-align:center;position:relative}
.th-step:first-child{flex:0 0 auto;text-align:left}
.th-step:last-child{flex:0 0 auto;text-align:right}

.th-dot{
  width:26px;height:26px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;
  font-size:12px;font-weight:800;background:var(--bg-card-hover);color:var(--text-muted);
  border:2px solid var(--border-subtle-2);position:relative;z-index:2
}
.th-step.feita .th-dot{background:var(--success-color);border-color:var(--success-color);color:#06240f}
.th-step.ativa .th-dot{
  background:var(--success-color);border-color:var(--success-color);color:#06240f;
...
.th-step:not(:first-child)::before{
  ...
  background:var(--border-subtle-2);z-index:1
}
.th-step.linha-feita:not(:first-child)::before{background:var(--success-color)}

.th-label{display:block;font-size:11px;color:var(--text-secondary);margin-top:6px;white-space:nowrap}
.th-step.feita .th-label,.th-step.ativa .th-label{color:var(--text-primary);font-weight:600}
</style>
</head>
<body>
          ...
          <div class="card mb-20">
            ...
            <div class="pedido-header">
              ...
              <c:choose>
                ...
                <c:when test="${p.status=='em_preparo'}"><span class="badge badge-warning">Preparando</span></c:when>
                <c:when test="${p.status=='pronto'}"><span class="badge badge-success">Pronto</span></c:when>
                <c:when test="${p.status=='entregue'}"><span class="badge" style="background:var(--bg-card-hover);color:var(--text-secondary)">Entregue</span></c:when>
            ...
            <c:if test="${p.status != 'cancelado'}">
              ...
              <c:set var="passo2Feita" value="${p.status=='pronto' || p.status=='entregue'}" />
              ...
              <div class="timeline-h">
                ...
                <div class="th-step ${passo2Feita ? 'feita linha-feita' : (passo2Ativa ? 'ativa linha-feita' : '')}">
                  <span class="th-dot"><c:choose><c:when test="${passo2Feita}">✓</c:when><c:otherwise>2</c:otherwise></c:choose></span>
                  <span class="th-label">Em Preparo</span>
                </div>
                <div class="th-step ${passo3Feita ? 'feita linha-feita' : (passo3Ativa ? 'ativa linha-feita' : '')}">
                  <span class="th-dot"><c:choose><c:when test="${passo3Feita}">✓</c:when><c:otherwise>3</c:otherwise></c:choose></span>
                  <span class="th-label">Pronto</span>
                </div>
                <div class="th-step ${passo4Ativa ? 'ativa linha-feita' : ''}">
                  <span class="th-dot"><c:choose><c:when test="${p.status=='entregue'}">✓</c:when><c:otherwise>4</c:otherwise></c:choose></span>
                  <span class="th-label">Entregue</span>
                </div>
              </div>
            </c:if>
            ...
            <div class="pedido-footer">
              ...
              <c:if test="${not empty p.observacao}">
                <span style="font-size:12px;color:var(--text-secondary)">📝 ${p.observacao}</span>
              </c:if>
            </div>
