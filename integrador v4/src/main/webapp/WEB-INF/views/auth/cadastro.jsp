...
<%--
    ================================================================
    CADASTRO.JSP — CRIAR CONTA DE CLIENTE
    ================================================================
     CORREÇÃO DE SEGURANÇA (aproveitando que já estava mexendo
...
<body>
	<div class="card">
		...
		<%}%>
		<form method="POST"
			action="${pageContext.request.contextPath}/auth/cadastro"
			onsubmit="return validar()">
			<div class="form-group">
				<label>Nome completo <span class="ob">*</span></label>
           ...
           comentário do topo do arquivo. --%>
				<input type="text" name="nome" placeholder="Seu nome"
					value="${fn:escapeXml(param.nome)}" required autofocus
					maxlength="100">
			</div>
			<div class="form-group">
				<label>Login <span class="ob">*</span></label> <input type="text"
					name="login" placeholder="Ex: joao.silva"
					value="${fn:escapeXml(param.login)}" required minlength="3"
					maxlength="50" onkeydown="if(event.key==' ')event.preventDefault()"
					autocomplete="username"> <span class="dica">Mínimo 3
					caracteres, sem espaços.</span>
			</div>
			<div class="form-group">
				<label>Senha <span class="ob">*</span></label> <input
					type="password" id="s1" name="senha" placeholder="••••••••"
					required minlength="6" oninput="validarSenhas()"
					autocomplete="new-password">
			</div>
			<div class="form-group">
				<label>Confirmar senha <span class="ob">*</span></label> <input
					type="password" id="s2" name="confirmarSenha"
					placeholder="••••••••" required minlength="6"
					oninput="validarSenhas()" autocomplete="new-password">
			</div>
			<button type="submit" class="btn-cadastrar">Criar conta</button>
		</form>
		<div class="link-login">
			Já tem conta? <a href="${pageContext.request.contextPath}/auth/login">Faça
				login</a>
		</div>
	</div>
	<script>
