...
<%--
    ...
    Mudança visual: adicionei um badge de status ("Recebido" em azul
    para quem ainda não começou / "Em Preparo" em âmbar para quem já
    foi iniciado) no topo do card, reproduzindo o badge colorido que
    aparece em cada card do Figma. A lógica de qual botão mostrar
    (Iniciar vs Concluir) continua exatamente a mesma de antes.
    ================================================================
--%>
<%
...
<div class="fila-card <%= isUrgente ? "urgente" : "" %>">
    ...
    <div class="fila-meta">
        ...
        <% if (isUrgente) { %>
            <span style="color:var(--error-color);font-weight:700">🔴 Urgente</span>
        <% } %>
        <span class="tempo">⏱ ~<%= tempo %> min</span>
        <span class="text-muted">desde <%= entrada %></span>
    </div>

    <% if (!isAguardando && operador != null && !operador.isEmpty()) { %>
        <div class="fila-preparando-por">👨‍🍳 Em preparo por <%= operador %></div>
    <% } %>
