...
<%--
    ...
    ================================================================
    Duas mudanças em relação à v4:

    1. REMOVIDO o card "Ações Rápidas" (Ver Mesas / Pedidos / Fila /
       Cardápio / Relatórios). Ele duplicava exatamente os mesmos
       links que já existem na sidebar, sempre visíveis — manter os
       dois só ocupava espaço e não agregava nada. A sidebar já
       resolve a navegação.

    2. OS GRÁFICOS FINALMENTE FICARAM REAIS. Na v4 eu tinha deixado
       de fora o gráfico "Receita da Semana" e a rosca "Vendas por
       Categoria" do Figma porque o DashboardController ainda não
       calculava esses dados — e preferi não desenhar um gráfico
       bonito com números inventados. Agora o Controller expõe
       `receitaPorDia` (Map com os últimos 7 dias) e `vendasPorSetor`
       (Map com Pratos/Bebidas/Sobremesas), então os gráficos abaixo
       são desenhados a partir de dados REAIS do banco.
...
<style>
.graficos-row{display:grid;grid-template-columns:2fr 1fr;gap:20px;margin-bottom:24px;align-items:stretch}
@media (max-width:900px){.graficos-row{grid-template-columns:1fr}}

.chart-eixo-label{font-size:10px;fill:var(--text-muted);font-family:inherit}
.chart-grid-line{stroke:var(--border-subtle);stroke-width:1}
.chart-linha{fill:none;stroke:var(--primary);stroke-width:2.5}
.chart-area{fill:url(#gradienteReceita);opacity:.25}

.rosca-wrap{display:flex;align-items:center;gap:20px;justify-content:center;height:100%}
.legenda-setores{display:flex;flex-direction:column;gap:10px;font-size:13px}
.legenda-item{display:flex;align-items:center;gap:8px}
.legenda-item .dot{width:9px;height:9px;border-radius:50%;flex-shrink:0}
.legenda-item .pct{margin-left:auto;font-weight:700;color:var(--text-primary);padding-left:14px}
</style>
</head>
<body>
  ...
  <main class="content">
    ...
    <div class="cards-grid">
      ...
      <div class="stat-card">
        ...
        <div class="stat-value" style="font-size:22px">
          R$ <fmt:formatNumber value="${ticketMedioHoje}" minFractionDigits="2" maxFractionDigits="2"/>
        </div>
    ...
    <%
        ...
        if (_receita != null) {
            for (BigDecimal v : _receita.values()) if (v.compareTo(_max) > 0) _max = v;
        }
        ...
        long _passoY = 1000;
        while (_passoY * 4 < _max.doubleValue()) _passoY *= 2;
        ...
        if (_receita != null && !_receita.isEmpty()) {
            ...
            for (Map.Entry<String, BigDecimal> _e : _receita.entrySet()) {
                ...
                _rotulosX.append("<text x='").append(_xs).append("' y='").append(_chartH - 4)
                         .append("' text-anchor='middle' class='chart-eixo-label'>")
                         .append(_e.getKey()).append("</text>");
                _i++;
            }
        }
        ...
        for (int _g = 0; _g <= 4; _g++) {
            ...
            String _ys = String.format(Locale.US, "%.1f", _y);
            _gradeY.append("<line x1='").append(_padL).append("' y1='").append(_ys)
                   .append("' x2='").append(_chartW - _padR).append("' y2='").append(_ys)
                   .append("' class='chart-grid-line'/>");
            _gradeY.append("<text x='").append(_padL - 8).append("' y='").append(_ys)
                   .append("' text-anchor='end' dominant-baseline='middle' class='chart-eixo-label'>")
                   .append("R$").append(String.format(Locale.US, "%.1f", _valor / 1000.0)).append("k</text>");
        }

        // ---------- GRÁFICO DE ROSCA: "Vendas por Categoria" ----------
        BigDecimal _totalSetor = BigDecimal.ZERO;
        if (_vendasSetor != null) {
            for (BigDecimal v : _vendasSetor.values()) _totalSetor = _totalSetor.add(v);
        }
        ...
        if (_vendasSetor != null) {
            ...
            for (Map.Entry<String, BigDecimal> _e : _vendasSetor.entrySet()) {
                double _pct = _totalSetor.compareTo(BigDecimal.ZERO) > 0
                        ? _e.getValue().doubleValue() / _totalSetor.doubleValue() : 0;
                double _tamanhoArco = _pct * _circunferencia;
                String _cor = _coresSetor[_si % _coresSetor.length];

                _fatias.append("<circle cx='").append(_cx).append("' cy='").append(_cy)
                       .append("' r='").append(_raio).append("' fill='none' stroke='").append(_cor)
                       .append("' stroke-width='").append(_espessura)
                       .append("' stroke-dasharray='")
                       .append(String.format(Locale.US, "%.2f", _tamanhoArco)).append(" ")
                ...
                _legendaSetor.append("<div class='legenda-item'><span class='dot' style='background:")
                             .append(_cor).append("'></span>").append(_e.getKey())
                             .append("<span class='pct'>").append(_pctArred).append("%</span></div>");
                _si++;
            }
        }
      ...
      <div class="card">
        ...
        <c:choose>
          <c:when test="${empty receitaPorDia}">
            <div class="empty-state"><div class="icon">📈</div><p>Sem dados de vendas ainda.</p></div>
          </c:when>
          <c:otherwise>
            <svg viewBox="0 0 <%= _chartW %> <%= _chartH %>" style="width:100%;height:auto">
              <defs>
      ...
      <c:choose>
        ...
        <c:otherwise>
          <div class="table-wrapper">
            <table>
              <thead><tr><th>Pedido</th><th>Cliente</th><th>Itens</th><th>Status</th><th>Total</th><th>Hora</th></tr></thead>
              <tbody>
                <c:forEach var="p" items="${ultimosPedidos}">
                  <tr>
                    <td><strong>#${p.idPedido}</strong>
                      <c:if test="${p.urgente}"><span class="badge badge-urgente" style="margin-left:6px">Urgente</span></c:if>
                    </td>
                    <td><c:choose><c:when test="${not empty p.mesa}">Mesa ${p.mesa.numero}</c:when><c:otherwise>${p.identificadorOperador}</c:otherwise></c:choose></td>
                    ...
                    <td style="color:var(--success-color);font-weight:700">
                      R$ <fmt:formatNumber value="${p.calcularTotal()}" minFractionDigits="2" maxFractionDigits="2"/>
                    </td>
